...
with open(filename, 'w', encoding='utf-8', newline='') as output_file:
    # writer = csv.DictWriter(output_file, quoting=csv.QUOTE_NONNUMERIC, fieldnames=columns)
    writer = csv.DictWriter(output_file, quoting=csv.QUOTE_NONNUMERIC, fieldnames=columns, extrasaction='ignore')
    # writer = csv.DictWriter(output_file, quoting=csv.QUOTE_NONNUMERIC)  # 尝试了一下，写的时候必须要有fieldnames
    writer.writeheader()
    # for row in medals_table:
    #     writer.writerow(row)
    # writer.writerows(medals_table)
    # writer.writerows(sorted(medals_table))  # 这个是不可行的，因为字典之间没有<来进行比较的功能。
    writer.writerows(sorted(medals_table, key=sort_key))  # key是针对便利对象元素的函数。